...
Switch the root attribute to `obsidian` or `lux` without changing component markup. The website’s `script.js` is the reference for the optional pointer-reactive `--mx`, `--my`, `--glow-opacity`, magnetic offset and material-lab controls.
